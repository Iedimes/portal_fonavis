            ...
            <div class="col-12">
                <h4>
                    ...
                    <a href="{{ url('projects/create') }}" class="announce">
                        <button class="btn btn-primary float-right" type="button"><i class="fa fa-fw fa-plus"></i> Crear
                            Proyecto</button>
                    </a>
                </h4>
            </div>
            ...
            <div class="row mb-3">
                <div class="col-md-6">
                    <input type="text" name="search" class="form-control"
                        placeholder="Buscar proyecto por código o nombre" value="{{ request('search') }}">
                </div>
                <div class="col-md-2">
                            ...
                            <td>
                                @if (isset($project->getEstado->stage_id))
                                    <label for="" class="text-green">
                                        {{ $project->getEstado->stage_id ? $project->getEstado->getStage->name : '' }}</label>
                                @else
                                    ...
                                    <div class="dropdown-menu" role="menu" style="">
                                        @if (isset($project->getEstado->stage_id) && $project->getEstado->stage_id == 10)
                                            <a class="dropdown-item"
                                                href="{{ url('projectsDocTec/' . $project->id) }}">Documentos VTA y ETH</a>
                                        @elseif (isset($project->getEstado->stage_id) && $project->getEstado->stage_id == 14)
                                            <a class="dropdown-item"
                                                href="{{ url('docObservados/' . $project->id) }}">Documentos obs. DIGH</a>
                                        @elseif (isset($project->getEstado->stage_id) && $project->getEstado->stage_id == 17)
                                            <a class="dropdown-item"
                                                href="{{ url('docObservadosDSGO/' . $project->id) }}">Documentos obs.
                                                DSGO</a>
                                        @else
                                            <a class="dropdown-item" href="{{ url('projects/' . $project->id) }}">Ver</a>
                                        @endif
                                        <a class="dropdown-item {{ $project->getEstado ? 'disabled' : '' }} "
                                            href="{{ url('projects/' . $project->id . '/edit') }}">Editar</a>
                                        <div class="dropdown-divider"></div>
                                        <a class="dropdown-item {{ $project->getEstado && $project->getEstado->stage_id == 7 ? '' : 'disabled' }}"
                                            href="{{ url('projects/' . $project->id . '/postulantes') }}">Grupo Familiar</a>
                                    </div>
                                </div>
                            </td>
                        </tr>
